...
XWindowsScreen::XWindowsScreen (const char* displayName, bool isPrimary,
    ...
    try {
        ...
        saveShape ();
        m_window      = openWindow ();
        m_screensaver = new XWindowsScreenSaver (
            m_display, m_window, getEventTarget (), events);
        m_keyState = new XWindowsKeyState (m_display, m_xkb, events, m_keyMap);
        LOG ((CLOG_DEBUG "screen shape: %d,%d %dx%d %s",
              m_x,
              m_y,
              m_w,
              m_h,
    ...
    m_events->adoptHandler (Event::kSystem,
                            ...
                            new TMethodEventJob<XWindowsScreen> (
                                this, &XWindowsScreen::handleSystemEvent));

    // install the platform event queue
    m_events->adoptBuffer (
        new XWindowsEventQueueBuffer (m_display, m_window, m_events));
}
...
XWindowsScreen::enter () {
    ...
    CARD16 powerlevel;
    BOOL enabled;
    if ((DPMSQueryExtension (m_display, &dummy, &dummy) != 0) &&
        (DPMSCapable (m_display) != 0) &&
        (DPMSInfo (m_display, &powerlevel, &enabled) != 0)) {
...
XWindowsScreen::setClipboard (ClipboardID id, const IClipboard* clipboard) {
    ...
    Time timestamp =
        XWindowsUtil::getCurrentTime (m_display, m_clipboard[id]->getWindow ());
...
XWindowsScreen::setOptions (const OptionsList& options) {
    for (UInt32 i = 0, n = options.size (); i < n; i += 2) {
        if (options[i] == kOptionXTestXineramaUnaware) {
            ...
            LOG ((CLOG_DEBUG1 "XTest is Xinerama unaware %s",
                  m_xtestIsXineramaUnaware ? "true" : "false"));
        } else if (options[i] == kOptionScreenPreserveFocus) {
            m_preserveFocus = (options[i + 1] != 0);
            LOG ((CLOG_DEBUG1 "Preserve Focus = %s",
                  m_preserveFocus ? "true" : "false"));
        }
    }
}
...
void
XWindowsScreen::reconfigure (UInt32 /*activeSides*/) {
    // do nothing
}
...
XWindowsScreen::registerHotKey (KeyID key, KeyModifierMask mask) {
    ...
    if ((mask & ~(KeyModifierShift | KeyModifierControl | KeyModifierAlt |
                  KeyModifierSuper)) != 0) {
    ...
    bool err = false;
    {
                ...
                for (int k = 0; k < modKeymap->max_keypermod && !err; ++k) {
                    ...
                    if (modifiermap[k] != 0) {
                        XGrabKey (m_display,
                                  ...
                                  GrabModeAsync);
                        if (!err) {
                            hotKeys.emplace_back (code, modifiers2);
        ...
        // those.
            ...
            for (auto j = keycodes.begin (); j != keycodes.end () && !err;
                 ++j) {
                for (size_t i = 0; i < (1u << numToggleModifiers); ++i) {
                    ...
                    XGrabKey (m_display,
                              *j,
                              ...
                              GrabModeAsync);
                    if (!err) {
                        hotKeys.emplace_back (*j, tmpModifiers);
    ...
    if (err) {
        ...
        for (auto& hotKey : hotKeys) {
            XUngrabKey (m_display, hotKey.first, hotKey.second, m_root);
            m_hotKeyToIDMap.erase (HotKeyItem (hotKey.first, hotKey.second));
        }

        m_oldHotKeyIDs.push_back (id);
        m_hotKeys.erase (id);
        LOG ((CLOG_WARN "failed to register hotkey %s (id=%04x mask=%04x)",
              synergy::KeyMap::formatKey (key, mask).c_str (),
              key,
              mask));
        return 0;
    }
...
XWindowsScreen::fakeMouseButton (ButtonID button, bool press) {
    ...
    if (xButton > 0 && xButton < 11) {
        XTestFakeButtonEvent (
            m_display, xButton, press ? True : False, CurrentTime);
        XFlush (m_display);
    }
}
...
XWindowsScreen::fakeMouseMove (SInt32 x, SInt32 y) {
    if (m_xinerama && m_xtestIsXineramaUnaware) {
        ...
        XTestFakeMotionEvent (
            m_display, DefaultScreen (m_display), x, y, CurrentTime);
    }
    XFlush (m_display);
}
...
XWindowsScreen::fakeMouseRelativeMove (SInt32 dx, SInt32 dy) const {
    ...
    if (false && m_xtestIsXineramaUnaware) {
        //		XWarpPointer(m_display, None, m_root, 0, 0, 0, 0, x, y);
    } else {
...
XWindowsScreen::fakeMouseWheel (SInt32 /*xDelta*/, SInt32 yDelta) const {
    ...
    const unsigned int xButton =
        mapButtonToX (static_cast<ButtonID> ((yDelta >= 0) ? -1 : -2));
    if (xButton == 0) {
    ...
    if (yDelta < m_mouseScrollDelta) {
        LOG ((CLOG_WARN "Wheel scroll delta (%d) smaller than threshold (%d)",
              yDelta,
              m_mouseScrollDelta));
    }
...
XWindowsScreen::openDisplay (const char* displayName) {
...
#if HAVE_XKB_EXTENSION
    {
        ...
        if (XkbLibraryVersion (&major, &minor) != 0) {
            ...
            if (XkbQueryExtension (display,
                ...
                m_xkb = true;
                XkbSelectEvents (
                    display, XkbUseCoreKbd, XkbMapNotifyMask, XkbMapNotifyMask);
    ...
    int dummyError;
    m_xrandr =
        (XRRQueryExtension (display, &m_xrandrEventBase, &dummyError) != 0);
    if (m_xrandr) {
...
XWindowsScreen::openIM () {
    ...
    // only at the moment.
    XIMStyles* styles;
    if (XGetIMValues (im, XNQueryInputStyle, &styles, NULL) != nullptr ||
        styles == nullptr) {
    ...
    XIC ic =
        XCreateIC (im, XNInputStyle, style, XNClientWindow, m_window, NULL);
    if (ic == nullptr) {
    ...
    unsigned long mask;
    if (XGetICValues (ic, XNFilterEvents, &mask, NULL) != nullptr) {
...
Bool
XWindowsScreen::findKeyEvent (Display* /*unused*/, XEvent* xevent,
                              XPointer arg) {
    ...
    return (xevent->type == filter->m_event &&
            ...
            xevent->xkey.keycode == filter->m_keycode)
               ? True
               : False;
}

void
XWindowsScreen::handleSystemEvent (const Event& event, void* /*unused*/) {
    ...
    if (m_ic != nullptr) {
        ...
        if (XFilterEvent (xevent, DefaultRootWindow (m_display)) != 0) {
            if (xevent->type == KeyPress) {
                // add filtered presses to the filtered list
                m_filtered.insert (m_lastKeycode != 0u);
            }
            return;
        }
        ...
        if (xevent->type == KeyRelease &&
            m_filtered.count (xevent->xkey.keycode != 0u) > 0) {
            m_filtered.erase (xevent->xkey.keycode != 0u);
            return;
        }
    }
    ...
    if (m_xi2detected) {
        ...
        if ((XGetEventData (m_display, cookie) != 0) &&
            ...
            if (cookie->evtype == XI_RawMotion) {
                if (m_isPrimary) {
                    ...
                    unsigned int msk;
                    xmotion.same_screen = XQueryPointer (m_display,
                                                         m_root,
                                                         &xmotion.root,
                                                         &xmotion.subwindow,
                                                         &xmotion.x_root,
    ...
    switch (xevent->type) {
        ...
        case SelectionClear: {
            ...
            if (id != kClipboardEnd) {
                ...
                sendClipboardEvent (
                    m_events->forClipboard ().clipboardGrabbed (), id);
                return;
            }
        } break;
        ...
        case SelectionRequest: {
            ...
            ClipboardID id =
                getClipboardID (xevent->xselectionrequest.selection);
            if (id != kClipboardEnd) {
        ...
        default:
            ...
            if (m_xkb && xevent->type == m_xkbEventBase) {
                ...
                switch (xkbEvent->any.xkb_type) {
                    ...
                    case XkbStateNotify:
                        LOG ((CLOG_INFO "group change: %d",
                              xkbEvent->state.group));
                        m_keyState->setActiveGroup (
                            static_cast<SInt32> (xkbEvent->state.group));
                        return;
                }
            }
#endif
            ...
            if (m_xrandr) {
                if ((xevent->type == m_xrandrEventBase + RRScreenChangeNotify ||
                    ...
                    reinterpret_cast<XRRNotifyEvent*> (xevent)->subtype ==
                        RRNotify_CrtcChange) {
                    LOG ((CLOG_INFO "XRRScreenChangeNotifyEvent or "
                                    "RRNotify_CrtcChange received"));

                    // we're required to call back into XLib so XLib can update
                    // its internal state
                    ...
                    // we need to resize m_window, otherwise we'll get a weird
                    // problem where moving off the server onto the client
                    // causes the pointer to warp to the center of the server
                    // (so you can't move the pointer off the server)
                    if (m_isPrimary) {
...
XWindowsScreen::onKeyPress (XKeyEvent& xkey) {
    LOG ((CLOG_DEBUG1 "event: KeyPress code=%d, state=0x%04x",
          xkey.keycode,
          xkey.state));
    ...
    if (key != kKeyNone) {
        ...
        m_keyState->sendKeyEvent (
            getEventTarget (), true, false, key, mask, 1, keycode);

        // do fake release if this is a fake press
        if (isFake) {
            m_keyState->sendKeyEvent (
                getEventTarget (), false, false, key, mask, 1, keycode);
        }
    } else {
...
XWindowsScreen::onKeyRelease (XKeyEvent& xkey, bool isRepeat) {
    ...
    if (key != kKeyNone) {
        ...
        if (!isRepeat) {
            ...
            LOG ((CLOG_DEBUG1 "event: KeyRelease code=%d, state=0x%04x",
                  keycode,
                  xkey.state));
            m_keyState->sendKeyEvent (
                getEventTarget (), false, false, key, mask, 1, keycode);
            ...
            LOG ((CLOG_DEBUG1 "event: repeat code=%d, state=0x%04x",
                  keycode,
                  xkey.state));
            m_keyState->sendKeyEvent (
                getEventTarget (), false, true, key, mask, 1, keycode);
        }
    }
}
...
XWindowsScreen::onHotKey (XKeyEvent& xkey, bool isRepeat) {
    ...
    if (!isRepeat) {
        m_events->addEvent (
            Event (type, getEventTarget (), HotKeyInfo::alloc (i->second)));
    }
    return true;
}
...
XWindowsScreen::onMouseRelease (const XButtonEvent& xbutton) {
    ...
    if (button != kButtonNone) {
        ...
        sendEvent (m_events->forIPrimaryScreen ().wheel (),
                   WheelInfo::alloc (0, 120));
...
XWindowsScreen::onMouseMove (const XMotionEvent& xmotion) {
    LOG ((CLOG_DEBUG2 "event: MotionNotify %d,%d",
          xmotion.x_root,
          xmotion.y_root));
    ...
    if (xmotion.send_event != 0) {
        ...
        } while (xevent.xany.send_event == 0);
        cntr = 0;
    } else if (m_isOnScreen) {
...
XWindowsScreen::createBlankCursor () const {
    ...
    Cursor cursor =
        XCreatePixmapCursor (m_display, bitmap, bitmap, &color, &color, 0, 0);
...
void
XWindowsScreen::processClipboardRequest (Window requestor, Time time,
                                         Atom property) {
    // check every clipboard until one returns success
    for (auto& id : m_clipboard) {
        if (id != nullptr && id->processRequest (requestor, time, property)) {
            break;
        }
    }
}
...
XWindowsScreen::destroyClipboardRequest (Window requestor) {
    // check every clipboard until one returns success
    for (auto& id : m_clipboard) {
        if (id != nullptr && id->destroyRequest (requestor)) {
            break;
        }
    }
}
...
int
XWindowsScreen::ioErrorHandler (Display* /*unused*/) {
...
XWindowsScreen::doSelectEvents (Window w) const {
    ...
    Window rw, pw, *cw;
    unsigned int nc;
    if (XQueryTree (m_display, w, &rw, &pw, &cw, &nc) != 0) {
...
XWindowsScreen::mapKeyFromX (XKeyEvent* event) const {
    ...
    LOG ((
        CLOG_DEBUG2 "mapped code=%d to keysym=0x%04x", event->keycode, keysym));
...
XWindowsScreen::updateButtons () {
    ...
    UInt32 numButtons = XGetPointerMapping (m_display, nullptr, 0);
...
XWindowsScreen::grabMouseAndKeyboard () {
    ...
    do {
        // keyboard first
        do {
            result = XGrabKeyboard (m_display,
                                    m_window,
                                    True,
                                    GrabModeAsync,
                                    GrabModeAsync,
        ...
        // now the mouse --- use event_mask to get EnterNotify, LeaveNotify
        // events
        result = XGrabPointer (m_display,
                               m_window,
                               False,
                               event_mask,
                               GrabModeAsync,
...
XWindowsScreen::refreshKeyboard (XEvent* event) {
    ...
    if (m_xkb && event->type == m_xkbEventBase) {
        XkbRefreshKeyboardMapping (
            reinterpret_cast<XkbMapNotifyEvent*> (event));
    } else {
#else
...
XWindowsScreen::detectXI2 () {
    ...
    return XQueryExtension (
               m_display, "XInputExtension", &xi_opcode, &event, &error) != 0;
}
...
XWindowsScreen::selectXIRawMotion () {
    ...
    mask.mask =
        static_cast<unsigned char*> (calloc (mask.mask_len, sizeof (char)));
